...
import database.diagram.edit.commands.ForeignKeyCreateCommand;
import database.diagram.edit.commands.ForeignKeyReorientCommand;
...
public class TableItemSemanticEditPolicy extends
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (DatabaseVisualIDRegistry.getVisualID(incomingLink) == ForeignKeyEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (DatabaseVisualIDRegistry.getVisualID(outgoingLink) == ForeignKeyEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (DatabaseVisualIDRegistry.getVisualID(node)) {
			case TableTableColumnCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (DatabaseVisualIDRegistry.getVisualID(cnode)) {
	...
	protected Command getStartCreateRelationshipCommand(
		...
		if (DatabaseElementTypes.ForeignKey_4001 == req.getElementType()) {
